...
def test_real_scaffold_composes_clean(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # scope_discipline is runtime-only — present on runtime, absent on optimizer.
    assert "Scope discipline" in runtime.text
    assert "Scope discipline" not in optimizer.text
